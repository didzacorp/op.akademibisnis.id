var refMember = new DaftarObj2({
 ...
 showDirectTeam: function(idEdit) {
   var me = this;
   // errmsg = this.CekCheckbox();
   errmsg = "";
   if (errmsg == "") {
 ...
 showDirectTeamLevel2: function(idEdit) {
   var me = this;
   // errmsg = this.CekCheckbox();
   errmsg = "";
   if (errmsg == "") {
     var box = this.GetCbxChecked();
      //1030 >
     var cover = this.prefix + "_formcoverLevel2";
     addCoverPage2(cover, 1032, true, false);
     document.body.style.overflow = "hidden";
     $.ajax({
       type: "POST",
       data: $("#" + this.formName).serialize()+"&idEdit="+idEdit,
       url: this.url + "&tipe=showDirectTeamLevel2",
       success: function(data) {
         var resp = eval("(" + data + ")");
         if (resp.err == "") {
           document.getElementById(cover).innerHTML = resp.content;
 ...
 showDirectTeamLevel3: function(idEdit) {
   var me = this;
   // errmsg = this.CekCheckbox();
   errmsg = "";
   if (errmsg == "") {
     var box = this.GetCbxChecked();
      //1030 >
     var cover = this.prefix + "_formcoverLevel3";
     addCoverPage2(cover, 1034, true, false);
     document.body.style.overflow = "hidden";
     $.ajax({
       type: "POST",
       data: $("#" + this.formName).serialize()+"&idEdit="+idEdit,
       url: this.url + "&tipe=showDirectTeamLevel3",
       success: function(data) {
         var resp = eval("(" + data + ")");
         if (resp.err == "") {
           document.getElementById(cover).innerHTML = resp.content;
