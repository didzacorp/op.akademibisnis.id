...
class refMemberObj extends configClass
{
    ...
    function set_selector_other($tipe)
    {
        ...
        switch ($tipe) {
            ...
            case 'showDirectTeam': {
                $fm      = $this->showDirectTeam($idEdit);
                $cek     = $fm['cek'];
                $err     = $fm['err'];
                $content = $fm['content'];
                break;
            }
            case 'showDirectTeamLevel2': {
                $fm      = $this->showDirectTeamLevel2($idEdit);
                $cek     = $fm['cek'];
                $err     = $fm['err'];
                $content = $fm['content'];
                break;
            }
            case 'showDirectTeamLevel3': {
                $fm      = $this->showDirectTeamLevel3($idEdit);
                $cek     = $fm['cek'];
                $err     = $fm['err'];
                $content = $fm['content'];
                break;
...
" . $scriptload;
    ...
    function showDirectTeam($idMember)
    {
        ...
        $fieldInform       .=  $this->newRow(array(
                                $this->customColumn("DIRECT TEAM ".$getDataMember['nama']." <br><br>".$this->tableDownline($idMember),
                                 '',
                                '12','12','0'),
                               ));
    ...
    function showDirectTeamLevel2($idMember)
    {
        $cek                = '';
        $err                = '';
        $content            = '';
        ...
        // $idEdit             = $_REQUEST[$this->Prefix . '_cb'];
        $getDataMember = sqlArray(sqlQuery("select * from member where id = '$idMember'"));
        $getNamaMemberUpline = sqlArray(sqlQuery("select * from member where id = '".$getDataMember['upline_level_1']."'"));

        $fieldInform       .=  $this->newRow(array(
                                $this->customColumn("DIRECT TEAM ".$getNamaMemberUpline['nama']." -> ".$getDataMember['nama']." <br><br>".$this->tableDownlineLevel2($idMember),
                                 '',
                                '12','12','0'),
                               ));
        ...
        // "<input type='button' class='btn btn-success btn-sm' value='Simpan' onclick ='" . $this->Prefix .".saveNew()' > ".
        "<input type='button' class='btn btn-success btn-sm' value='Tutup' onclick ='" . $this->Prefix .".CloseLevel2()' >"
    ...
    function showDirectTeamLevel3($idMember)
    {
        $cek                = '';
        $err                = '';
        $content            = '';
        ...
        $getDataMember = sqlArray(sqlQuery("select * from member where id = '$idMember'"));
        $getNamaMemberUpline = sqlArray(sqlQuery("select * from member where id = '".$getDataMember['upline_level_1']."'"));
        $getNamaMemberUpline2 = sqlArray(sqlQuery("select * from member where id = '".$getDataMember['upline_level_2']."'"));

        $fieldInform       .=  $this->newRow(array(
                                $this->customColumn("DIRECT TEAM ".$getNamaMemberUpline2['nama']." -> ".$getNamaMemberUpline['nama']." -> ".$getDataMember['nama']." <br><br>".$this->tableDownlineLevel3($idMember),
                                 '',
                                '12','12','0'),
                               ));
        ...
        // "<input type='button' class='btn btn-success btn-sm' value='Simpan' onclick ='" . $this->Prefix .".saveNew()' > ".
        "<input type='button' class='btn btn-success btn-sm' value='Tutup' onclick ='" . $this->Prefix .".CloseLevel3()' >"
    ...
    function tableDownline($idMember){
      $className= "row0";
      $nomor = 1;
      ...
      while ($dataMember = sqlArray($getDataMember)) {
        ...
        <tr class='$className'>
          ...
          <td class='GarisDaftar' style='text-align:right;valign:middle;'>$jumlahDirectTeam</td>
          <td class='GarisDaftar' style='text-align:center;valign:middle;'>
          <input type='button' class='btn btn-success' value='DETAIL' onclick=$this->Prefix.showDirectTeamLevel2($id)>
          </td>
        </tr>
        ";
        if($nomor % 2 == 1){
      ...
      <thead>
       <tr style='background: #1094f7;color: white;border-bottom: 2px solid #f55757;'>
         ...
         <th class='th01' style='text-align:center;vertical-align:middle;width:30%;'>Direct Team</th>
    ...
    function tableDownlineLevel2($idMember){
      $className= "row0";
      $nomor = 1;
      $getDataMember = sqlQuery("select * from member where upline_level_1 = '".$idMember."' ");
      while ($dataMember = sqlArray($getDataMember)) {
        ...
        <tr class='$className'>
          ...
          <td class='GarisDaftar' style='text-align:right;valign:middle;'>$jumlahDirectTeam</td>
          <td class='GarisDaftar' style='text-align:center;valign:middle;'>
          <input type='button' class='btn btn-success' value='DETAIL' onclick=$this->Prefix.showDirectTeamLevel3($id)>
          </td>
        </tr>
        ";
        if($nomor % 2 == 1){
    ...
    function tableDownlineLevel3($idMember){
      $className= "row0";
      $nomor = 1;
      $getDataMember = sqlQuery("select * from member where upline_level_1 = '".$idMember."' ");
      while ($dataMember = sqlArray($getDataMember)) {
        ...
        $headerTable  = "<thead>
    	   <tr style='background: #1094f7;color: white;border-bottom: 2px solid #f55757;'>
      	   ...
      	   $Checkbox
    		   ...
    		   <th class='th01'  width='100'   style='text-align:center;vertical-align:middle;'>DIRECT TEAM</th>
    		   <th class='th01'  width='100'   style='text-align:center;vertical-align:middle;'>TEAM</th>
    	   </tr>
	        </thead>";
