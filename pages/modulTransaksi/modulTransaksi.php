        ...
        if ($err == '') {
          ...
          if($getDataTransaksi['jenis_transaksi'] == 'PENJUALAN'){
            ...
            if(($getDataTransaksi['status'] == 'BELUM BAYAR' || $getDataTransaksi['status'] == 'MENUNGGU KONFIRMASI' ) && $statusTransaksi == 'TERKONFIRMASI'){
              ...
              $queryUpdateTransaksi = "UPDATE transaksi set status = 'TERKONFIRMASI', update_time = now() where id = '$idEdit'";
              sqlQuery($queryUpdateTransaksi);
              $explodeEmailMember = explode("@",$getDataTransaksi['email_pembeli']);
              $userNameMember = substr($explodeEmailMember[0], 0, 6).$idEdit;
              $dataMember = array(
              ...
              $getIdMember = sqlArray(sqlQuery("select max(id) from member where email='".$getDataTransaksi['email_pembeli']."'"));
              $dataUsers = array(
              ...
              $subjectEmail = "Pembayaran Pendaftaran Member Terkonfirmasi" ;
              $bodyEmail = "
              <p>&nbsp;</p>
              <div class='container' style='text-align: center; font-family: monospace; font-size: 16px;'>
              <div class='row justify-content-center'>
...
" . $scriptload;
    ...
    function genDaftarOpsi()
    {
        ...
        // $this->sendEmailKonfirmasi("dzakirharista@gmail.com","test email","<b> asu </b>");
